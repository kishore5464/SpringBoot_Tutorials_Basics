...
@Table(name = "employee")
@NamedQueries(value = {
		@NamedQuery(name = "Employee.findByDepartment", query = "SELECT e FROM Employee e WHERE e.department = ?1"),
		@NamedQuery(name = "Employee.getLast_NameOrDegree", query = "SELECT emp FROM Employee emp WHERE emp.lastName = ?1 OR emp.degree = ?2") })
